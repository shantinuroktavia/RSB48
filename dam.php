...
class DAM extends mysqli{
	public function __construct(){
		if($_SERVER['SERVER_NAME']=="localhost")
			parent::__construct("localhost", "root", "", "buku-kuliah");
		else
	...
	public function retrieve($tableName, $data){
		if($tableName == "pengguna"){
			if($data === true){
				...
				$whereStatement = substr($whereStatement, 0, strlen($whereStatement)-4);
				$query = "SELECT `ID`,`isAdmin`,`Username`,`Nama`,`Email`,`Password`, `Lokasi` as `LokasiID`,`nama_lokasi` AS Lokasi,`Reputasi`,`Deskripsi`,`MulaiBlokir`,`SelesaiBlokir`,`AlasanBlokir`,`URLFoto`,`isAlphaTester`,`Jumlah_Rater`, `FirstUploadFlag`, `FifthUploadFlag`, `FirstFullRatingFlag`, `FifthFullRatingFlag` FROM `aktor_sistem` LEFT JOIN `lokasi` ON `lokasi`.`id_lokasi` = `aktor_sistem`.`Lokasi` WHERE $whereStatement";
			}
			//var_dump($query);exit(0);
			return parent::query($query);
		}else if($tableName == "pesan"){
